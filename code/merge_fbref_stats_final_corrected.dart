...
void main() async {
  ...
  final outputFile = File('merged_2025_26_with_fbref_final.csv');

  print('🔧 Starting FINAL CORRECTED FBRef statistics merge...');
  ...
  // We need to map by column index, not by name (since names are duplicated)
  ...
  for (var i = 2; i < fantacalcioLines.length; i++) {
    ...
    if (parts.length >= 4) {
      ...
      if (fbrefPlayer != null) {
        ...
        final mp = fbrefPlayer['MP'] ?? '';
        final starts = fbrefPlayer['Starts'] ?? '';
        final min = fbrefPlayer['Min'] ?? '';
        final games90 = fbrefPlayer['90s'] ?? '';

        // Get TOTAL stats from the first occurrence of each column
        final glsTotal =
            fbrefPlayer['Gls'] ??
            ''; // This will be the first occurrence (total)
        final astTotal =
            fbrefPlayer['Ast'] ??
            ''; // This will be the first occurrence (total)
        final gaTotal =
            fbrefPlayer['G+A'] ??
            ''; // This will be the first occurrence (total)
        final gpk = fbrefPlayer['G-PK'] ?? '';
        final pk = fbrefPlayer['PK'] ?? '';
        final pkatt = fbrefPlayer['PKatt'] ?? '';
        final crdy = fbrefPlayer['CrdY'] ?? '';
        final crdr = fbrefPlayer['CrdR'] ?? '';
        final xg = fbrefPlayer['xG'] ?? '';
        final npxg = fbrefPlayer['npxG'] ?? '';
        final xag = fbrefPlayer['xAG'] ?? '';
        final npxgxag = fbrefPlayer['npxG+xAG'] ?? '';
        final prgc = fbrefPlayer['PrgC'] ?? '';
        final prgp = fbrefPlayer['PrgP'] ?? '';
        final prgr = fbrefPlayer['PrgR'] ?? '';
  ...
  print('\n=== FINAL CORRECTED MERGE RESULTS ===');
  print('✅ Final corrected merged file created: ${outputFile.path}');
  ...
  if (sampleMatches.isNotEmpty) {
    print('\nSample matches (with FINAL CORRECTED data):');
  ...
  print('\n🎉 FINAL CORRECTED FBRef statistics merge completed!');
...
Future<void> createUpdatedPositionFiles(
  ...
  print('\n=== CREATING FINAL CORRECTED POSITION FILES ===');
  ...
  for (final entry in playersByPosition.entries) {
    ...
    // Create filename
    final fileName = 'giocatori_${ruolo.toLowerCase()}_final.csv';
    ...
    // Show top 3 players for this position with FINAL CORRECTED FBRef stats
    if (players.isNotEmpty) {
      print('   Top players (FINAL CORRECTED data):');
  ...
  print('\n🎉 FINAL CORRECTED POSITION FILES CREATED!');
