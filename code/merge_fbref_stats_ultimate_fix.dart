...
void main() async {
  ...
  final outputFile = File('merged_2025_26_with_fbref_ultimate.csv');

  print('🔧 Starting ULTIMATE FIX FBRef statistics merge...');
  ...
  print('\n=== ULTIMATE FIX MERGE RESULTS ===');
  print('✅ Ultimate fix merged file created: ${outputFile.path}');
  ...
  if (sampleMatches.isNotEmpty) {
    print('\nSample matches (with ULTIMATE FIX data):');
...
Future<void> createUpdatedPositionFiles(
  ...
  print('\n=== CREATING ULTIMATE FIX POSITION FILES ===');
  ...
  for (final entry in playersByPosition.entries) {
    ...
    // Create filename
    final fileName = 'giocatori_${ruolo.toLowerCase()}_ultimate.csv';
    ...
    // Show top 3 players for this position with ULTIMATE FIX FBRef stats
    if (players.isNotEmpty) {
      print('   Top players (ULTIMATE FIX data):');
  ...
  print('\n🎉 ULTIMATE FIX POSITION FILES CREATED!');
