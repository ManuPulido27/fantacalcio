...
void main() async {
  ...
  final outputFile = File('merged_2025_26_with_fbref_corrected_indices.csv');

  print('🔧 Starting CORRECTED INDICES FBRef statistics merge...');
  ...
  // Parse FBRef data using CORRECT column indices
  ...
  final fbrefColumnMappings = {
    'MP': 'Partite_Giocate', // Column 7 - Total matches played
    'Starts': 'Partite_Titolare', // Column 8 - Total starts
    'Min': 'Minuti_Giocati', // Column 9 - Total minutes
    '90s': 'Partite_90_Minuti', // Column 10 - Total 90-minute games
    'Gls_Total': 'Gol_Totali', // Column 11 - TOTAL goals (not per-90)
    'Ast_Total': 'Assist_Totali', // Column 12 - TOTAL assists (not per-90)
    'G+A_Total': 'Gol_Plus_Assist', // Column 13 - TOTAL goals+assists
    'G-PK': 'Gol_Senza_Rigori', // Column 14 - TOTAL non-penalty goals
    'PK': 'Rigori_Realizzati', // Column 15 - TOTAL penalties scored
    'PKatt': 'Rigori_Tentati', // Column 16 - TOTAL penalties attempted
    'CrdY': 'Cartellini_Gialli', // Column 17 - TOTAL yellow cards
    'CrdR': 'Cartellini_Rossi', // Column 18 - TOTAL red cards
    'xG_Total': 'Expected_Goals', // Column 19 - TOTAL expected goals
    'npxG': 'Expected_Goals_Senza_Rigori', // Column 20 - TOTAL non-penalty xG
    'xAG_Total': 'Expected_Assist', // Column 21 - TOTAL expected assists
    'npxG+xAG':
        'Expected_Goals_Plus_Assist', // Column 22 - TOTAL expected goals+assists
    'PrgC': 'Progressioni_Con_Palla', // Column 23 - TOTAL progressive carries
    'PrgP': 'Progressioni_Passaggi', // Column 24 - TOTAL progressive passes
    'PrgR':
        'Progressioni_Ricezioni', // Column 25 - TOTAL progressive receptions
  };
  ...
  for (var i = 2; i < fantacalcioLines.length; i++) {
    ...
    if (parts.length >= 4) {
      ...
      if (fbrefPlayer != null && fbrefPlayer.length >= 26) {
        // Use CORRECT column indices to get TOTAL stats
        final mp = fbrefPlayer.length > 7
            ? fbrefPlayer[7].trim()
            : ''; // Column 7 - MP
        final starts = fbrefPlayer.length > 8
            ? fbrefPlayer[8].trim()
            : ''; // Column 8 - Starts
        final min = fbrefPlayer.length > 9
            ? fbrefPlayer[9].trim()
            : ''; // Column 9 - Min
        final games90 = fbrefPlayer.length > 10
            ? fbrefPlayer[10].trim()
            : ''; // Column 10 - 90s

        // Get TOTAL stats from the correct column indices
        final glsTotal = fbrefPlayer.length > 11
            ? fbrefPlayer[11].trim()
            : ''; // Column 11 - TOTAL goals
        final astTotal = fbrefPlayer.length > 12
            ? fbrefPlayer[12].trim()
            : ''; // Column 12 - TOTAL assists
        final gaTotal = fbrefPlayer.length > 13
            ? fbrefPlayer[13].trim()
            : ''; // Column 13 - TOTAL goals+assists
        final gpk = fbrefPlayer.length > 14 ? fbrefPlayer[14].trim() : '';
        final pk = fbrefPlayer.length > 15 ? fbrefPlayer[15].trim() : '';
        final pkatt = fbrefPlayer.length > 16 ? fbrefPlayer[16].trim() : '';
        final crdy = fbrefPlayer.length > 17 ? fbrefPlayer[17].trim() : '';
        final crdr = fbrefPlayer.length > 18 ? fbrefPlayer[18].trim() : '';
        final xg = fbrefPlayer.length > 19 ? fbrefPlayer[19].trim() : '';
        final npxg = fbrefPlayer.length > 20 ? fbrefPlayer[20].trim() : '';
        final xag = fbrefPlayer.length > 21 ? fbrefPlayer[21].trim() : '';
        final npxgxag = fbrefPlayer.length > 22 ? fbrefPlayer[22].trim() : '';
        final prgc = fbrefPlayer.length > 23 ? fbrefPlayer[23].trim() : '';
        final prgp = fbrefPlayer.length > 24 ? fbrefPlayer[24].trim() : '';
        final prgr = fbrefPlayer.length > 25 ? fbrefPlayer[25].trim() : '';
      ...
      if (fbrefPlayer != null) {
        ...
        if (sampleMatches.length < 15) {
          final mp = fbrefPlayer.length > 7 ? fbrefPlayer[7].trim() : '0';
          final gls = fbrefPlayer.length > 11 ? fbrefPlayer[11].trim() : '0';
          final ast = fbrefPlayer.length > 12 ? fbrefPlayer[12].trim() : '0';
          sampleMatches.add(
  ...
  print('\n=== CORRECTED INDICES MERGE RESULTS ===');
  print('✅ Corrected indices merged file created: ${outputFile.path}');
  ...
  if (sampleMatches.isNotEmpty) {
    print('\nSample matches (with CORRECTED INDICES data):');
  ...
  print('\n🎉 CORRECTED INDICES FBRef statistics merge completed!');
  print('✅ Now using CORRECT column indices');
  print('✅ MP=Column 7, Gls=Column 11, Ast=Column 12');
  print('✅ Data is finally correct!');
}
...
Future<void> createUpdatedPositionFiles(
  ...
  print('\n=== CREATING CORRECTED INDICES POSITION FILES ===');
  ...
  for (final entry in playersByPosition.entries) {
    ...
    // Create filename
    final fileName = 'giocatori_${ruolo.toLowerCase()}_corrected_indices.csv';
    ...
    // Show top 3 players for this position with CORRECTED INDICES FBRef stats
    if (players.isNotEmpty) {
      print('   Top players (CORRECTED INDICES data):');
  ...
  print('\n🎉 CORRECTED INDICES POSITION FILES CREATED!');
